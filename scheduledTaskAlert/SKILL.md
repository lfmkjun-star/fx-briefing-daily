...
## 3단계 — (알림 대상이 있을 때만) 배경 뉴스 조사 및 선별

신규 알림 대상 통화에 대해서만 WebSearch로 당일 급변동 배경을 찾는다. USD는 연준·CPI·달러인덱스·지정학 이벤트, JPY는 BOJ·엔 캐리트레이드·안전자산 흐름 위주로 검색한다. 넉넉하게(5~8개 후보) 모은 뒤, `fx-briefing-daily` 작업의 "여섯 번째 관점: 기사 큐레이터" 기준을 그대로 적용해 최종 목록을 확정한다:

- **신뢰도**: 로이터·블룸버그·연합뉴스·연합인포맥스·매일경제·한국경제·Investing.com 등 공신력 있는 언론사만 채택. 출처 불명·자극적 제목의 커뮤니티/블로그성 글, 루머성 기사는 제외.
- **중복 제거**: 같은 사건·같은 수치를 거의 동일하게 다룬 기사는 가장 자세하거나 가장 먼저 보도한 것 하나만 남긴다.
- **목표 개수**: 통화당 **3~5개**를 목표로 하되, 기준 통과분이 3개 미만이면 그만큼만 남긴다(0개도 허용). 이 목록이 6단계 HTML용이고, 5단계 카카오톡용은 이 중 최대 2개만 추가로 뽑는다.

실제로 읽은 기사의 URL·매체명·제목·2~3문장 요약만 기록한다 (지어낸 내용·지어낸 링크 절대 금지).
...
2. **알림 대상 통화의 탭만** 갱신한다 (다른 통화 탭은 건드리지 않음):
   ...
   - `.rate-value`, `.delta-pill`(방향 클래스 포함), `#rateSubUsd`/`#rateSubJpy`, 레인지 차트 SVG(마커 위치 재계산: `30 + (현재값-52주최저)/(52주최고-52주최저)*(570-30)`), "오늘 왜 움직였나"/"다음 며칠 전망" 문단, `.note`(반드시 `is-warning` 클래스, 상태 배지는 "주의" 이상)를 모두 오늘 데이터로 교체한다. (세부 필드 위치는 `fx-briefing-daily` 작업과 동일한 HTML 구조를 참고.)
   - `.sources` 목록 — 3단계에서 확정한 목록 전부(3~5개, 부족하면 있는 만큼)로 교체. `.source-item`을 기사 수만큼 반복해서 추가/삭제한다.
   - `#genDate`는 "YYYY-MM-DD (요일) 마지막 갱신"으로 갱신.
   - `<footer>`의 안내 문구는 그대로 둔다 (지우거나 고치지 않는다).
